"""
05_build_panel.py  [STOXX 50 PROXY]
-------------------------------------
Build the monthly panel of market efficiency metrics.
For each (ticker, month), run OLS market model using ^STOXX50E as benchmark.

Key difference vs STOXX 600 analysis: MARKET_TICKER = "^STOXX50E"

Input:  stoxx50/data/intermediate/prices_raw.parquet
Output: stoxx50/data/intermediate/panel_monthly.parquet
"""
...
MIN_OBS = 15
MARKET_TICKER = "^STOXX50E"  # ← EURO STOXX 50 benchmark (vs ^STOXX for SX600)

# ============================================================
# 1. CHARGEMENT
# ============================================================
print(">>> Chargement des données...")
prices = pd.read_parquet(SX50_ROOT / "data" / "intermediate" / "prices_raw.parquet")
...
if MARKET_TICKER not in prices["ticker"].unique():
    raise ValueError(
        ...
        "Vérifiez que 02_collect_prices.py a bien téléchargé ^STOXX50E."
    )
...
out_path = SX50_ROOT / "data" / "intermediate" / "panel_monthly.parquet"
panel_df.to_parquet(out_path, index=False)
print(f"\n✅ Export : {out_path}")
print(f"   Colonnes : {panel_df.columns.tolist()}")
print(f"   Tickers  : {panel_df['ticker'].nunique()}")
print(
    f"   Fenêtre  : {panel_df['date'].min().date()} → {panel_df['date'].max().date()}"
)
