"""
panel_plots.py  [STOXX 50 PROXY]
----------------------------------
Generate a 3x2 summary figure of the EURO STOXX 50 proxy panel data.
...
Output: stoxx50/figures/panel_overview.png
"""
...
def main(show: bool = False):
    ...
    log("Chargement de stoxx50/data/intermediate/panel_monthly.parquet...")
    panel = pd.read_parquet(
        SX50_ROOT / "data" / "intermediate" / "panel_monthly.parquet"
    )
    log(f"panel chargé: {len(panel):,} lignes, colonnes={panel.columns.tolist()}")

    log("Chargement de stoxx50/data/intermediate/events.csv...")
    events = pd.read_csv(SX50_ROOT / "data" / "intermediate" / "events.csv")
    ...
    fig.suptitle(
        "EURO STOXX 50 Proxy (rank ≤ 50 in STOXX 600) — Market Efficiency Panel (2013–2026)",
        fontsize=14,
        fontweight="bold",
        y=1.01,
    )
    ...
    # 4. Number of ADD events per month (proxy SX50 inclusions)
    log("Trace 4/6: Nombre de ADD (SX50 proxy)")
    ax = axes[1, 1]
    add_counts = (
    ...
    ax.set_title("ADD events per rebalancing date (rank ≤ 50 proxy)")
    ax.set_ylabel("Count")
    ...
    panel_comp = pd.read_parquet(
        SX50_ROOT / "data" / "intermediate" / "panel_composition.parquet"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot panel overview charts (SX50 proxy)."
    )
    parser.add_argument(
        "--show",
        action="store_true",
